...
class TorontoOpenDataMonitor:
    ...
    def _fetch_ckan_records(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            LOGGER.warning(
                "Could not resolve Toronto CKAN package metadata; will try fallback CSV: %s",
                exc,
            )
        ...
        for resource in resources:
            ...
            if fmt in {"csv", "json", "geojson"} or url.lower().endswith((".csv", ".json", ".geojson")):
                urls_to_try.append(
                    (url, fmt or "csv", str(resource.get("name") or resource.get("id") or "resource"))
                )
        ...
        for url, fmt, label in urls_to_try:
            try:
                ...
                if records:
                    ...
                    LOGGER.info(
                        "Toronto Open Data loaded %d record(s) from %s",
                        len(records),
                        label,
                    )
                    return records
        ...
        for resource in [r for r in resources if r.get("datastore_active")]:
            try:
                ...
                LOGGER.warning(
                    "Could not read Toronto CKAN datastore resource %s: %s",
                    resource.get("id"),
                    exc,
                )

        raise RuntimeError(
            "No usable Toronto Open Data CSV/JSON/datastore resource found for development-applications"
        )
...
class NewsMonitor:
    ...
    def fetch_relevant_posts(self) -> list[dict[str, Any]]:
        ...
        lookback_days = int(self.config.get("lookback_days", 21))
        cutoff = utcnow() - timedelta(days=lookback_days)
        require_published_date = bool(self.config.get("require_published_date", False))
        ...
        for site in self.config.get("sites", []):
            ...
            for post in site_posts:
                ...
                if require_published_date and not published_dt:
                    continue
